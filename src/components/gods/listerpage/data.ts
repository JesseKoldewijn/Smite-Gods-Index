...
const getFallbackData = async () => {
  ...
  if (isServer) {
    return (await import('../../../../public/data/fallback-response.json').then(
      (module) => module.default
    )) as GodsResponse;
  }
  return (await fetch(`/data/fallback-response.json`).then((res) => {
    if (!res.ok) {
      console.error('Failed to fetch fallback data. Please check the file path.', res.status);
      return {
        data: [],
        meta: {
          pagination: {
...
const getGodData = async () => {
  ...
  if (!isExpired) {
    ...
    const hasCorrectShape = cachedData
      ? cachedData?.data?.data?.length > 0 && cachedData?.data?.meta?.pagination
      : false;
...
export const getGodsListerPageData = async () => {
  ...
  godsResponseMeta ??= fallbackData.meta as Omit<GodsResponse['meta'], 'undefined'>;
